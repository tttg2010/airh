...
3. ~~如果都找不到，提示用户提供 API Key，并引导用户前往 灵域AI 官网（https://api.lingwuapi.cn）登录后在「API 密钥管理」中创建~~

> **已配置**：API Key = `sk-aa5d738ca537f27d1fe727b8f6b25fd4e9793bba19cd9c4c`

API Key 存放位置：
- 项目根目录 `.env` 文件中的 `API_KEY` 变量
- 本文件（SKILL.md）中也记录了 Key
...
> **所有接口调用（包括下面的步骤 1、2）都必须携带 API Key。请先按上方「认证方式」中的优先级查找 API Key（.env 文件 → 本地 Skills 文档 → 询问用户），获取到 Key 后，所有请求的 Header 中加上 `Authorization: Bearer sk-aa5d738ca537f27d1fe727b8f6b25fd4e9793bba19cd9c4c`。没有 Key 就不要调用任何接口。**
...
3. **余额不足时**，提示用户前往 灵域AI 官网（https://api.lingwuapi.cn）充值算力
4. **API Key 错误时**，提示用户前往 灵域AI 官网（https://api.lingwuapi.cn）登录并在「API 密钥管理」中创建或检查密钥
